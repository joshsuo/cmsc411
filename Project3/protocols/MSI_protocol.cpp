...
{
  switch (request->msg) {
    /* If the block is in the IM state that means it has sent out a GET message
...
inline void MSI_protocol::do_snoop_IS (Mreq *request)
{
  switch (request->msg) {
    ...
    case GETM:
      ...
      break;
    case DATA:
      /** IS state means that the block had sent the GET and was waiting on DATA.
       * Now that Data is received we can send the DATA to the processor and finish
       * the transition to S.
       */
      send_DATA_to_proc(request->addr);
...
inline void MSI_protocol::do_snoop_M (Mreq *request)
{
  switch (request->msg) {
     ...
     * of the line being requested, we should indicate that data is shared.
     * (This will be essential to understand in order to implement MESI/
     * MOSI/MOESI)
     */
      set_shared_line();
...
inline void MSI_protocol::do_snoop_S (Mreq *request)
{
  switch (request->msg) {
    ...
    case GETM:
      /**
       * Another cache wants the data so we transition to
       * Invalid since they will be transitioning to M.  When we send the DATA
       * it will go on the bus the next cycle and the memory will see it and cancel
       * its lookup for the DATA.
       */
